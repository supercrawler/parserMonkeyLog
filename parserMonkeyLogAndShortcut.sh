...
img_count=$3
devices="$4"
if [[ $((total_num/100/img_count)) -eq 0 ]]; then
       ...
       # mm=`find_keyword "$line" "$SEND_EVENT_PATTERN"`
        ...
        if [[ -n $mm ]]; then
            ...
            if [[ $cur_num -ne $pre_num ]]; then
                #echo "$event_num : $line"
                value=$((cur_num+offset))
                echo "${SEND_EVENT_PATTERN} #${value}"
